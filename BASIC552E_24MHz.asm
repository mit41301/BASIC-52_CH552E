...
SCON1		EQU	0C0H	; UART1 control register
U1TI		BIT	0C0H+1	; UART1 transmit interrupt flag
U1RI		BIT	0C0H+0	; UART1 receive interrupt flag
SBUF1		EQU	0C1H	; UART1 data register
SBAUD1		EQU	0C2H	; UART1 baud rate setting register
;=====================
;
	;**************************************************************
	;
...
;-- CH552E Insert Start ---------------------------------------------------------
;===============================================================================
;  CH552E Set Fixed baudrate 19.23Kbps
;  SBAUD1 = 256 - (Fsys/16/Baud Rate) : When Fsys = 24MHz, SCON1 U1SMOD=1
;===============================================================================
baud19K	equ	178				; SBAUD1 Value of 19.23Kbps
;
BG1:
	mov		t2con,#00h		; Timer2 not use as baudrate generater
	mov		scon,#00h		; UART0 not use

	mov		scon1,#32h		; Fast baud rate, Receive enable, Transmit interrupt flag on
	mov		sbaud1,#baud19k	; set baud rate
...
TXX:	JNB	U1TI,$		;WAIT FOR TRANSMIT READY
	CLR	U1TI
	MOV	SBUF1,R5 	;SEND OUT THE CHARACTER
;	=== CH552E end ===
	;
...
;	MOV	A,SBUF
;	CLR	RI		;RESET READY
INCHAR2:JNB	U1RI,$		;WAIT FOR RECEIVER READY.
	MOV	A,SBUF1
	CLR	U1RI		;RESET READY
;	=== CH552E end ===
	CLR	ACC.7		;NO BIT 7
	;
...
SDA		bit	P1.4                    ;I2C serial data line.
